...
export default function ServiceEditor({ services, onSave, onDelete }: ServiceEditorProps) {
    ...
    return (
        <div className="flex h-[calc(100vh-140px)] gap-6 overflow-hidden">
                    ...
                    `}
                        >
                            <div className="flex justify-between items-center mb-1">
                                <span className={`font-bold text-base ${selectedId === service.id ? 'text-brand-amber' : 'text-brand-light'}`}>
                                    {service.name}
                                </span>
                                {service.category === 'principal' && <span className="h-2 w-2 rounded-full bg-brand-amber" title="Servicio Principal" />}
                            </div>
            ...
            {/* 2. Editor Form (Centro - Ahora Panoramico) */}
                ...
                {editingService ? (
                    <div className="space-y-8 animate-fade-in max-w-4xl mx-auto">
                        ...
                        <div className="flex justify-between items-start border-b border-brand-stone/30 pb-6">
                            <div>
                                <h2 className="text-3xl font-black uppercase text-brand-light">Editar Servicio</h2>
                                <div className="flex items-center gap-2 mt-2">
                                    <span className="text-xs font-mono text-brand-light/40 bg-brand-stone/20 px-2 py-1 rounded">ID: {editingService.id}</span>
                                    <span className={`text-xs uppercase tracking-widest px-2 py-1 rounded border ${editingService.category === 'principal' ? 'border-brand-amber text-brand-amber' : 'border-brand-stone text-brand-stone'
                                        }`}>
                            ...
                            <button
                                onClick={() => onDelete(editingService.id)}
                        ...
                        <div className="grid grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                <div>
                                    <label className="block text-xs uppercase tracking-widest text-brand-light/60 mb-2 font-bold">Nombre del Servicio</label>
                                    <input
                                        type="text"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs uppercase tracking-widest text-brand-light/60 mb-2 font-bold">Duración</label>
                                        <input
                                            type="text"
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs uppercase tracking-widest text-brand-light/60 mb-2 font-bold">Precio (Texto)</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs uppercase tracking-widest text-brand-light/60 mb-2 font-bold">Precio (Numérico)</label>
                                        <input
                                            type="number"
                                ...
                                <div>
                                    <label className="block text-xs uppercase tracking-widest text-brand-light/60 mb-2 font-bold">Descripción Corta</label>
                                    <textarea
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-brand-stone/50 animate-pulse">
                        ...
                        <p className="uppercase tracking-widest font-bold">Selecciona un servicio para editar</p>
                    </div>
                )}
            </div>
        </div>
